...
const STAGING_CATALOG = [
  ...
  { name: 'Himalayan Koh Pink Salt Pouches', category: 'Uncategorized' },
  { name: 'HIMALAYAN ROCK SALT BAG 18 LBS', category: 'Bulk Order' },
  { name: 'HIMALAYAN SALT POUCHES', category: 'Bulk Order' },
  { name: 'HIMALAYAN CRYSTAL ROCK SALT LAMP IONIZER AIR PURIFIER WITH DIMMABLE CORD', category: 'Uncategorized' },
...
describe('isNicheProduct', () => {
  ...
  it('drops the three animal-feed products, by name or by category', () => {
  ...
  it('treats a missing category as no evidence of being off-niche', () => {
...
describe('sectionsWithProducts', () => {
  it('lists only shelves that actually hold something, with real counts', () => {
    ...
    expect(sections.find((section) => section.key === 'edible-pink-salt')?.count).toBe(5);
