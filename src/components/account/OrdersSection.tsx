...
/**
 * The customer's own orders — list on the left, selected order on the right.
 *
 * Extracted from the standalone `/orders` page, which is now the My Orders tab
 * of the account portal (`/account?tab=orders`). The page chrome and the
 * account sidebar stayed behind, so this renders as a section inside the
 * portal's layout instead of duplicating it — the two screens used to be two
 * copies of the same fetch, badge and reorder logic in two different files.
 */
...
export default function OrdersSection() {
  ...
  if (loading) return <SkeletonOrderList count={3} />;
